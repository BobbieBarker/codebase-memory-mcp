...
TEST(resolve_qualified_suffix_sees_through_arity_fence) {
    ...
    cbm_resolution_t a =
        cbm_registry_resolve_ctx(r, "Fx.Store.fetch", "proj.lib.caller", &one, NULL, NULL, 0);
    ASSERT_STR_EQ(a.qualified_name, "proj.lib.store.Fx.Store.fetch#1");

    cbm_resolve_ctx_t three = {.container_qn = NULL, .arity = 3};
    cbm_resolution_t b =
        cbm_registry_resolve_ctx(r, "Fx.Store.fetch", "proj.lib.caller", &three, NULL, NULL, 0);
...
TEST(resolve_same_module_uses_caller_container) {
    ...
    cbm_resolution_t bare = cbm_registry_resolve(r, "fetch", "proj.lib.store", NULL, NULL, 0);
...
TEST(resolve_foreign_receiver_does_not_bind_local_name) {
    ...
    cbm_resolution_t ok = cbm_registry_resolve(r, "Fx.Probe.get", "proj.lib.store", NULL, NULL, 0);
 ...
 *
 * The receiver-chain rule is applied to the CONTAINER probe only, so it reaches
 * a language that opts in by setting container_qn and no other. This pins the
 * half that did NOT change: through the NULL-context entry point there is no
 * container, the file-QN probe answers, and a foreign receiver still binds to a
 * same-named local exactly as it does on main. The earlier version of this test
 * asserted the opposite, because the guard ran on both probes; the maintainer's
 * standing rule is that a call-resolution change of this kind is per-language,
 * so the file-QN probe went back to its original behaviour and this test went
 * with it. Whether to extend the rule to every grammar is a separate question
 * with its own evidence still owed. */
TEST(resolve_receiver_guard_is_scoped_to_the_container_probe) {
    cbm_registry_t *r = cbm_registry_new();
    ...
    /* Through the NULL-context entry point there is no container, so the
     * file-QN probe answers and a foreign receiver binds, as on main. These
     * three are the pre-existing behaviour this commit deliberately preserves
     * for the other 161 grammars. */
    cbm_resolution_t jv =
        cbm_registry_resolve(r, "LocalDate.parse", "proj.com.acme.Order", NULL, NULL, 0);
    ASSERT_STR_EQ(jv.qualified_name, "proj.com.acme.Order.parse");

    cbm_resolution_t py =
        cbm_registry_resolve(r, "Template.render", "proj.app.views", NULL, NULL, 0);
    ASSERT_STR_EQ(py.qualified_name, "proj.app.views.render");

    cbm_resolution_t js = cbm_registry_resolve(r, "Math.max", "proj.web.cart", NULL, NULL, 0);
    ASSERT_STR_EQ(js.qualified_name, "proj.web.cart.max");

    /* With a container passed, the guard fires and withholds the foreign
     * receiver. This is the half the commit adds, reachable only where
     * cbm_lang_container_is_source_named admits the language.
     *
     * module_qn has to differ from container_qn for this to prove anything. The
     * container probe finds proj.com.acme.Order.parse and the guard refuses it,
     * but refusal falls through to the file-QN probe, which is deliberately
     * unguarded now; if module_qn also held `parse` that probe would hand back
     * the same candidate and the guard would look inert. Elixir never has them
     * equal, because container_qn is the enclosing module's QN and module_qn is
     * the file's, and pass_calls.c withholds the container when the two match. */
    cbm_resolve_ctx_t gx = {.container_qn = "proj.com.acme.Order", .arity = CBM_ARITY_NONE};
    cbm_resolution_t guarded =
        cbm_registry_resolve_ctx(r, "LocalDate.parse", "proj.com.acme", &gx, NULL, NULL, 0);
    ASSERT_TRUE(!guarded.qualified_name || guarded.qualified_name[0] == '\0');
...
SUITE(registry) {
    ...
    RUN_TEST(resolve_receiver_guard_is_scoped_to_the_container_probe);

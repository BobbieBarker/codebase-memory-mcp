...
static int resolve_single_call(cbm_pipeline_ctx_t *ctx, CBMCall *call,
    ...
    /* The arity hint is withheld outside a language that overloads by arity:
     * a fenced candidate is reachable from any caller's bare name through
     * qualified_suffix_match and sole_fenced_candidate, so an ungated hint lets a
     * bare Java or Python callee bind to an Elixir arity in a polyglot repo.
